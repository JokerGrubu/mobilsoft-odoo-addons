...
export class SnippetEditorDialog extends Component {
  setup() {
    ...
    onWillStart(() => {
      if (this.props.bodyProps.data) this.state.data = this.props.bodyProps.data;
    });
  ...
  confirm_edit() {
    ...
    this.props.bodyProps.save(this.state.data);
    this.close_dialog()
  }
  selectCol(col) {
  ...
  hasSaveData() {
    ...
    if (!isDataValid) {
      ...
      this.props.bodyProps.save(this.props.bodyProps.data)
