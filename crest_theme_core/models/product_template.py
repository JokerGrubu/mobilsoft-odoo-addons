...
class ProductTemplate(models.Model):
    ...
    free_qty = fields.Float(
        ...
        digits='Available'  # fix typo here
    )
    ...
    def check_inventory_config(self):
        ...
        # get a warehouse for the current website or fallback to first company warehouse
        warehouse = website.warehouse_id or self.env['stock.warehouse'].search(
            [('company_id', '=', website.company_id.id)],
            limit=1
        )
        if (
            not self.sudo().allow_out_of_stock_order
            and self.sudo().with_context(warehouse=warehouse.id).free_qty < 1
        ):
            show_stock_label = True

        return show_stock_label
    ...
    def _compute_qty_dict_bits(self):
        ...
        available_variants = {pv['id']: pv for pv in self.sudo().with_context(warehouse=warehouse.id).product_variant_ids.read(['free_qty']) }
        prod_available = {}
        for product in self:
            free_qty = 0
