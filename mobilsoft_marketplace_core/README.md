...
`mobilsoft_marketplace_core` modülü, Odoo üzerinde tüm e-ticaret pazaryeri entegrasyonları için temel framework ve base sınıfları sağlar.
...
1. Modülü aktif et:
```bash
odoo -d MobilSoft -i mobilsoft_marketplace_core
```
...
```python
from mobilsoft_marketplace_core.connectors import BaseMarketplaceConnector
...
```python
'depends': ['mobilsoft_marketplace_core', ...]
```
...
python -m pytest addons/mobilsoft_marketplace_core/tests/

# Spesifik test
python -m pytest addons/mobilsoft_marketplace_core/tests/test_connector.py
```
